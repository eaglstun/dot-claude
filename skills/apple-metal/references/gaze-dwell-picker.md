...
---
# Gaze-raycast + dwell select in a stereo renderer (head-pose → hit-test → ring)
...
- ARKit `ARCamera.transform` — column-major 4x4, camera→world; **−Z (column 2 negated) is the
  ...
  world position. Verified in practice by deriving forward as `-columns.2`.
...
The menu tiles are baked premultiplied-alpha plates drawn through a premultiplied blend (src `.one`,
...
the Swift-side menu uniform struct must stay byte-identical to its MSL counterpart.
...
This is the recipe for any gaze+dwell UI (a content picker, an in-scene reticle

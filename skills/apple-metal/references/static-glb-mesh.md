...
Shape of the implementation: a `GlbMesh` parser, a `texturedVertex`/`texturedFragment` shader pair,
and a textured pipeline state. Spec context: `~/.agents/references/gen-3d/native-skinning-runtime.md` (Material +
...
`.glb` under a synchronized root group in the Xcode project (e.g. `GenAssets/`) is auto-added as a

...
- Depth facts this builds on: `apple-arkit/references/scene-depth.md` (r32Float, metres,
  ~256x192, registered, optical-axis Z, 60 Hz).

**Provenance:** generalised from a working per-eye trail effect — a per-eye trail texture array,
a fade pass (`.blendColor` destination multiply), a per-frame contribution pass (`.max`), and an
additive composite (`.one`/`.one`).
...
1. **Single shared drawable-sized buffer, isolated by viewport (mode-4's scheme).** One
   ...
   Eyes stay independent **iff every fragment reads only its own
...
So the decay is a separate fullscreen pass:
...
Tuning lives as named constants in house style: e.g.

...
**Provenance:** diagnosed in a stereo passthrough renderer that drew both eyes in one encoder,
with the camera Y/CbCr planes bound once before the eye loop and a mid-loop composite pass
rebinding fragment slot 0. The pattern generalises to any shared-encoder loop.
...
- **Give transient passes their own non-shared slot** (e.g. composite samples a slot no per-eye
  backdrop relies on). Watch for collisions: in the renderer this came from, 0/1 = camera, 2 = depth, 3 = text/

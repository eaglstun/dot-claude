...
- Spec: `~/.agents/references/gen-3d/native-skinning-runtime.md`
- glTF 2.0 spec §3.6 (Accessors), §5.21 (skin), §3.7 / §5.16 (animation), §3.6.2.2 (cubic spline)
- Shape of the implementation: a `GltfSkinnedLoader` + `SkinnedModel` pair over a shared `GlbCore`
  parser, drawn through a `skinnedVertex`/`skinnedFragment` shader pair.
...
floor**. Two-part fix (model placement + a resolved floor height):

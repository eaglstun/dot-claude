...
- Working precedents from a shipped phone-GPU stereo renderer:
- `cubeVertex`/`cubeFragment` — the flat prop path:
...
- A procedural mesh builder whose emitters ALREADY compute smooth per-vertex normals
...
- Pipeline creation with no vertex descriptor (every prop
...
- `fragment-effect-fusion-and-cost.md` — the mode-2 blur (~17 taps/eye ×2)
  cost anchor; `float3`-in-uniform padding trap; per-eye fusion rules.
- `depth-stylization.md` — fast-math `normalize(0)` NaN guard (default
...
parallax** and fuses — the same argument that makes a view-space radial-normal disco ball safe.
...
an `fogFbm`-style hash noise); **grunge/fabric** = fbm multiply; **stripes/
...
same as the textured path). **Cost: 1 tap/px ×2 eyes** — trivial vs the mode-2 anchor.

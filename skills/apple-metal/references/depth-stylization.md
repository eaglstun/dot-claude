...
---
# Edge / normal / stylization recipes for a depth-passthrough renderer
...
- Working precedents these recipes build on, from a shipped AR depth-passthrough renderer:
  - `depthOrFar` — depth in metres, with `<=0` (no LiDAR reading) sunk to 8 m so foreground
    silhouettes against sky/windows still register.
  - a 4-tap N/S/E/W depth-gradient silhouette.
  - `outlineOnlyFragment` — 24-tap (DIRS 8 × RINGS 3) feathered depth-edge disc, anti-aliasing the
    low-res LiDAR boundary into a glow.
  - `sampleCameraRGB` — YCbCr→RGB at one UV (reusable).
  - `depthPalette` — near→far cosine ramp.

Fetched / verified: 2026-06-28.

## Depth facts that constrain every edge effect here

(Full detail in `apple-arkit/references/scene-depth.md`.) `smoothedSceneDepth.depthMap` is
...
1. **Derivative (cheapest, ~free):** unproject the centre depth texel to a camera-space position
   `p` (the same pinhole math the depth-reprojection vertex function uses), then
...
**Fast-math caveat (default ON in Metal pipelines):** `normalize` of a near-zero vector

...
**Provenance:** an AR passthrough renderer that stamps real-world LiDAR depth into the z-buffer
via a fullscreen prepass (`lessEqual` + depth **write**), after which virtual props draw with the
same depth state and fail the test wherever a real surface is nearer. Assumes the depth buffer
clears to far (1.0) each frame — `depthStencilPixelFormat = .depth32Float`, default `clearDepth`.
...
Shape of the implementation: a `stampRealDepth(_:)` step and a dedicated depth-prepass pipeline,
`sceneDepth` (buildDepthStates), call sites in `drawBackground` (default branch + case 3);
with a `depthOnlyFragment` paired with the reprojection vertex function.

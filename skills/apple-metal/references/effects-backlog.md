...
can auto-cycle on a ~12 s timer; a controller button still cycles
...
- **Comic-book ink + halftone** _(M, very high)_ — ink lines + CMYK dot-screen fill; the
  "pop-art" mode. Outline lane's favorite; #2 is its stepping-stone.
- **Depth Portal / Stage backdrop** _(M→L, very high)_ — keep the near subject, replace the far
  room with a synthwave stage. The showpiece. Ship screen-locked, then world-lock via per-pixel
...
If the host app exposes a music clock (**`bpm` + `meter`**) plus timed cue
cues (`lines[].t`). Plan: a small clock that uses `bpm` when present (`beatPeriod=60/bpm`,
downbeat every `meter`-numerator beats, anchored to playback start / a timed cue), and falls back to
...
the host app's own API contract.

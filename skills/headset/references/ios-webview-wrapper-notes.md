...
The app defaults to its configured production URL, where the build emits a
...
persisted with `@AppStorage("serverHost")`, so
...
- **Deploy to the paired phone from the CLI** (no Xcode GUI needed):
  ```sh
  # 1. build signed — use your Apple Developer team identifier

...
chord (catches fret-arithmetic errors). It then writes:
    ...
    <song-dir>/bass.md            (per-section bass tablature: each lead.md row
                                   tabbed as its bass line, with lyrics and
                                   inline bass cues)
...
from chart_lib import (
    ...
    bass_target, primary_position, parse_lead, render_flow_bass_tab,
)
...
# ---------------------------------------------------------------------------
# Render bass.md — per-section tablature
#
# The bass chart follows lead.md top to bottom: same sections, same rows, same
# lyrics. Each chord row is rendered as an actual bass tab (one column per bar,
# the chord's bass note at its go-to low-neck fret) instead of a chord grid,
# and the notes live in each section rather than in one table up top.
# Bass-specific info still rides inline: an italic `_bass: ..._` cue under any
# row with slash chords, plus hand-authored feel cues from chart.py's BASS_CUES.
# ---------------------------------------------------------------------------

# one source line — flow files render soft breaks as real breaks in the PDF
PREAMBLE_BASS = (
    "Standard tuning, low→high: **E A D G**. Each section below is bass "
    "tablature — one column per bar, read left to right. Play the chord root "
    "(on slash chords `X/Y`, the slash note `Y`) at the fret shown; frets are "
    "the go-to low-neck position. Inline `bass:` and feel cues flag anything "
    "that isn't a plain root.\n"
)

# annotations like `_(fade out)_` or `*(break — 2 bars)*` in a lead.md row
_ANNOT_RE = re.compile(r"[*_]\(([^)]*)\)[*_]")
...
def _row_annotations(raw: str) -> list[str]:
    """Italic `_(...)_` / `*(...)*` notes in a lead row (e.g. 'fade out')."""
    return _ANNOT_RE.findall(raw)
...
def render_bass_md(song: Song, lead_text: str,
    ...
    for sec in sections:
        ...
        for row in sec.rows:
            if row.chords:
                out.extend(["", "```", render_flow_bass_tab(row.chords), "```"])
            else:
                # chordless bar(s): a rest, or a "(synth/pad)"-style note
                notes = _row_annotations(row.raw) or ["tacet"]
                out.extend(["", "  ".join(f"_({n})_" for n in notes)])

            if row.lyric:
                out.append(f"_{row.lyric}_")
            ...
            # trailing play-notes on a chord row (e.g. "fade out" after Bb)
            if row.chords:
                for n in _row_annotations(row.raw):
                    out.append(f"_({n})_")

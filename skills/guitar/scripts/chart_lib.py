...
# ---------------------------------------------------------------------------
# Sequential bass tab — one row of the lead-sheet-flow bass chart
# ---------------------------------------------------------------------------

def render_flow_bass_tab(chords: list[str]) -> str:
    """Sequential bass tab for one lead-sheet row.

    One column per chord, kept in playing order (duplicate bars included). Each
    chord's bass note — the slash bass on `X/Y`, else the root — is placed at
    its go-to low-neck fret on the correct string. Returns five lines: a
    chord-name label row over the four-string staff (G/D/A/E, high→low), e.g.
        ...
        E |--------3---|
    """
    cells = []  # (name, string_letter, fret_str)
    for name in chords:
        string, fret = parse_position(primary_position(bass_target(name)))
        cells.append((name, string, str(fret)))
    widths = [max(len(name), len(fret), 2) for name, _, fret in cells]

    label = "   "  # sits under the "G |" staff prefix
    for (name, _, _), w in zip(cells, widths):
        label += " " + name.ljust(w)
    lines = [label.rstrip()]

    for letter in ("G", "D", "A", "E"):  # high → low, the way bass tab stacks
        row = f"{letter} |"
        for (_, string, fret), w in zip(cells, widths):
            row += "-" + (fret.ljust(w, "-") if string == letter else "-" * w)
        row += "|"
        lines.append(row)
    return "\n".join(lines)

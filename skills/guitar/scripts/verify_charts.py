...
bass.md (per-section tablature format):
  - Every fret written in a tab block produces that chord's bass target
    note (root, or slash bass) in standard bass tuning (E A D G).
  - Each tab block covers exactly its chords' bass targets — one tabbed
    note per bar.
  - Section headers and the per-bar chord sequence agree with the song's
    lead.md (the bass chart mirrors it).
...
_STAFF_RE = re.compile(r"^\s*([EADG])\s*\|(.*)\|\s*$")


def _tab_blocks_by_section(text: str) -> List[Tuple[str, List[Tuple[List[str], List[str]]]]]:
    """Parse a per-section tablature bass.md.

    Returns [(section header, [(label chords, staff lines), ...]), ...] — one
    inner tuple per ``` fenced tab block, in document order.
    """
    ...
    while i < len(lines):
        ...
        if line.startswith("## "):
            ...
            if cur is not None and block:
                label_chords = [normalize_name(t) for t in block[0].split()]
                cur[1].append((label_chords, block[1:]))
        else:
            i += 1
    return sections
...
def audit_bass(path: Path) -> List[str]:
    """Audit a per-section tablature bass.md."""
    issues: List[str] = []
    text = path.read_text()
    sections = _tab_blocks_by_section(text)

    # --- per tab block: every fret produces its chord's bass target ---
    for header, blocks in sections:
        for label_chords, staff_lines in blocks:
            expected: List[str] = []
            for name in label_chords:
                try:
                    root, _, slash, _ = parse_chord(name)
                except Exception as e:
                    issues.append(f"  {header}: cannot parse chord {name!r} ({e})")
                    continue
                expected.append(slash if slash else root)  # already sharp-normalized
            produced: List[str] = []
            for sl in staff_lines:
                m = _STAFF_RE.match(sl)
                if not m:
                    continue
                letter = m.group(1)
                for run in _DIGITS_RE.findall(m.group(2)):
                    produced.append(note_at(letter, int(run)))
            if sorted(produced) != sorted(expected):
                issues.append(
                    f"  {header}: tab notes {sorted(produced)} don't match "
                    f"bass targets {sorted(expected)} for "
                    f"[{' '.join(label_chords)}]"
                )

    # --- sections + per-bar chord sequence mirror lead.md ---
    lead_path = path.parent / "lead.md"
    if lead_path.exists():
        lead_sections = _bass_sections(lead_path)
        # only chorded rows produce a tab block; chordless bars (rests) don't
        lead_map = [(s.header, [r.chords for r in s.rows if r.chords])
                    for s in lead_sections]
        bass_map = [(h, [lc for lc, _ in blocks]) for h, blocks in sections]
        ...
        if lead_headers != bass_headers:
            ...
            for (h, lrows), (_, brows) in zip(lead_map, bass_map):
                if lrows != brows:
                    issues.append(f"  section '{h}': chord sequence doesn't "
                                  f"mirror lead.md")
    else:
...
def bass_chord_names(path: Path) -> Set[str]:
    """Chord names appearing in a bass.md's tab-block labels."""
    names: Set[str] = set()
    for _, blocks in _tab_blocks_by_section(path.read_text()):
        for label_chords, _ in blocks:
            names.update(label_chords)  # already normalize_name'd
    return names

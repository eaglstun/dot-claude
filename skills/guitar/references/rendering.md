...
**Bar rows render as fixed-width measure boxes.** `bar_row_to_html()` in `preprocess()` turns each `lead.md` bar row into an HTML table of equal-width cells (`BAR_WIDTH_IN`, ~1.4in each — four of them ≈ 3/4 of the page), so bars line up visually across the whole sheet regardless of chord-name length. It also caps at four bars per line (splitting longer rows, independent of source line breaks) and renders a trailing `(N bars)` annotation bold beside the boxes. Keyed on lines starting with `|` and containing `//` beat marks; fenced blocks like bass tabs are skipped. To stretch every line to a full 3/4 width instead of keeping bar width uniform, switch the table `width` from `n * BAR_WIDTH_IN` to a constant. Flow files also render with hard line breaks (`hard_breaks=True` in `convert()`), so every source newline becomes a real line break — that keeps each lyric on its own line directly below its measure row instead of collapsing onto it.

...
Protomaps is a strong "host your own tiles" option for a static site plus CDN.
The whole basemap is **one file** (`.pmtiles`) you drop on object
...
## Why this fits a static-site stack

- A `.pmtiles` file is just a static asset → place it on a web server or object
  storage with a CDN. nginx supports range
    ...
    style: {
      ...
      sources: {
        protomaps: {
          ...
          url: "pmtiles://https://maps.example.net/region.pmtiles",
          attribution:
...
## 4. Host the `.pmtiles` file — two options

The browser just needs HTTP **range requests** + permissive **CORS**. Both
options below give you that. Decision rule:

- **Small/medium regional cutout (≲ ~1 GB), one or two sites** → put it on an
  existing nginx host. This is the simplest option if disk space is available.
- **Large file, or you want it off the web server's disk/bandwidth** → use
  S3-compatible object storage with a CDN edge.
...
> hosting notes). The real constraint is **disk**, not RAM: keep the
...
### Option A — an nginx host

1. **Upload** the file to the site's public directory:
   ```bash
   rsync -avz --progress region.pmtiles user@example.net:/var/www/maps/
   ```
2. **Add a `.pmtiles` location** to that vhost
   (edit the relevant nginx virtual host with sudo). The
...
3. **Validate and reload nginx**:
   ```bash
   ssh user@example.net 'sudo nginx -t && sudo systemctl reload nginx'
   ```
4. Your source URL becomes
   `pmtiles://https://maps.example.net/region.pmtiles`.

### Option B — S3-compatible object storage (+ CDN)

Use an S3-compatible CLI or the provider's control panel. Enable the optional
CDN edge so range requests hit the edge rather than the origin.
...
the API) allowing the application's HTTPS origin (or `*`), methods
...
spend the web server's modest disk/bandwidth on map tiles.

### Sanity checks after deploy (either option)

```bash
pmtiles show https://maps.example.net/region.pmtiles   # reads over HTTP via range reqs
curl -sI -H 'Range: bytes=0-99' https://maps.example.net/region.pmtiles \

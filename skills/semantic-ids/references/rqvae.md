...
Today's v2 `semantic_id` is a calibrated SimHash: 192 independent bits, no structure
between them. Flip one high bit and the document lands at the far end of the sort order
while staying semantically adjacent — which is exactly why prefix-bucketing is a trap. **RQ-VAE**
...
```yaml
semantic_id: "v2:0H0AFzKRsgoDcilfc5IC1eIVAcMTT4FD" # 192-bit Hamming code
related_ids: # unchanged by topic operations
...
Collisions are real and expected: two documents can quantize to the same topic code at
every level. `topic_id` is a bucket, not a unique key; use the separate 192-bit
`semantic_id` to identify and rank documents within that bucket.
...
`data/<context>.vectors.json`), so a bad training run costs nothing but time.
...
1. Embed a **large external corpus** in the same broad domain (technical documentation,
   ...
   worth considering: public docs corpora, your own repositories' markdown, a
...
3. **Freeze and version the codebooks.** Unlike v2 semantic IDs, topic IDs remain tied to
   a trained corpus artifact.
...
codebooks become **permanent versioned artifacts** — see below.
...
```toml
[topics]
codebook   = "../data/topics-v1.codebook.json"   # FROZEN and versioned
levels     = 4
codebook_k = 16
...
```bash
topics.py train                  # once per codebook version, then freeze
topics.py assign                 # stamp topic_id
topics.py label                  # name the buckets → topic_path
...
The v2 benchmark retained the exhaustive cosine winner **97.4%** of the time with the
default top-80 Hamming shortlist. That is why you should be honest that **this project buys
navigability, not accuracy.** Retrieval
...
Retraining the codebooks on a grown corpus **re-buckets every document**. A codebook is
opaque; you will not be able to eyeball whether a retrain was safe.
...
geometry of _paragraphs_ and was then asked to bucket _abstracts_ (`"Title. Summary."`).
...
**Bucket quality after v2**, by how much of a bucket comes from one source directory:
    ...
    F  20 docs  scheduling/messaging/web-hosting/music
...
The metric was wrong, not the bucket. Its _label_ is genuinely bad, though
...
- **Labels are the weak link, not the buckets.** Showing the model 25 titles and asking for
  one slug is thin. Showing it the titles _plus_ what distinguishes this bucket from its
...
to distinguish the buckets _from each other_, and enforce distinctness in code rather than
...
real cluster is _tight_. Ranked by that number, the bottom of the list was exactly the
buckets flagged by eye — with no access to the names:

    corpus baseline                     0.637
    0.658  FC  scheduling/hosting/messaging/network  → "calendar-sync"   ← confabulated
...
are _less_ similar to each other than two random documents are. And that is CORRECT: a

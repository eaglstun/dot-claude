...
lives in the project's experiment workspace (`npm run experiments` → `results/*.md`); the
...
V2 asks 192 deterministic random-hyperplane questions about that vector and keeps one sign
bit per answer, producing a compact code where **Hamming distance approximates semantic
distance**. The hyperplanes come from a SHA-256-derived seed; their origin is regenerated
from 65 fixed calibration sentences spanning technology, science, culture, relationships,
arts, health, work, home, and everyday life. No indexed corpus becomes encoder state.

## Why raw zero is not the origin
...
One subtraction recovered ~35 bits of real capacity in v1. It proved that an origin is
necessary, but not that an evolving corpus should define it forever.

V2 replaces the frozen corpus mean with a reproducible calibration suite embedded by the
configured model. On the 65-domain suite it measures **0 dead bits, 3.0% skew, and a
97.0-bit mean pair distance** against a 96-bit chance target. All 192 bits carry meaning;
there is no date/hash tail.
...
**ITQ won by a mile, and v1 used `truncate` anyway.** Two reasons, and both were about
...
That choice still left v1 dependent on a frozen 768-float corpus mean. V2 accepts SimHash's
lower raw ranking quality in exchange for a stateless, reconstructible encoder, then uses a
larger shortlist and cosine rescoring to recover precision.
...
| encoding      | chars  | total bits | semantic bits |
| ------------- | ------ | ---------- | ------------- |
| hex           | 32     | 128        | 128           |
| **base64url** | **32** | **192**    | **192**       |
...
192 bits is still a **coarse** instrument. The binary code is an excellent candidate
generator and a mediocre ranker.
...
1. **Sweep with the bits.** Cheap, coarse. Its only job is to not _lose_ the right answer.
   Takes the top `--shortlist` (default 80) by Hamming distance.
2. **Rescore the survivors with the full-precision float vectors.** Expensive, precise, and
   it only ever touches the shortlist.

On the 2026-09 cached-corpus benchmark, the v2 top-80 Hamming shortlist retained the exact
cosine winner **97.4%** of the time. This is why `data/<context>.vectors.json` exists.
`--no-rescore` turns rescoring off and shows the raw bit ranking.

The float vectors, calibration origin, and index are all **regenerable and disposable**.

## Considered alternative: cosine scan only

At this corpus size, the simplest retrieval system would be an exhaustive cosine scan over
the cached float embeddings. It would need no vector database, binary candidate stage, or
meaning-bearing ID: embed the query, compare it with every document vector, and sort. The
index is small enough that this is operationally reasonable, and every artifact would be
rebuildable from the source documents and embedding model.

That is **not the current decision**. Keep `semantic_id`. Its compact Hamming code gives a
document a portable, meaning-bearing identity; supports useful comparisons without loading
the float-vector cache; and lets `related_ids` travel with the document and survive a lost
index. Cosine rescoring remains the final ranker.

The cosine-only alternative remains rejected for now. V2 keeps semantic IDs while removing
corpus-derived encoder state through its fixed, cross-domain calibration suite.

## Things that would invalidate every ID in a context

- Changing the embedding model.
- Changing the projection seed or any calibration sentence.
- Changing `SEMANTIC_BITS` or the ID encoding.
- Changing what text gets embedded — the title/summary fields, or turning tags on or off.

Any of these requires a **new ID version and re-mint**, not a migration. IDs carry `v2:` so
the engine can refuse cross-version comparison instead of returning meaningless numbers.

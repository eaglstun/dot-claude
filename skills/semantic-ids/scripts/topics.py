...
Where `semantic_id` is 192 independent bits with no structure between them (flip a high bit
...
def encode_topic(codes: list[int], version: str) -> str:
    ...
    Retraining a codebook can feel routine even though it changes every assignment, which
    is exactly why the guard has to be inside the ID itself.
    """

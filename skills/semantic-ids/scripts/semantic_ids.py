#!/usr/bin/env python3
"""
Semantic IDs — a versioned 192-bit base64url string whose bits ARE the meaning
...
vector database, no server, nothing to deploy.
    ...
    v2:<32 base64url chars>
       └─ 192 deterministic SimHash bits over a calibrated embedding

This is the CONTEXT-DRIVEN engine. It knows nothing about any particular corpus; a
context TOML says where the files are and how to read a title/summary out of them.
See contexts/claude-home.toml.
...
Two properties this guarantees, because an ID committed to git is forever:
  ...
  IDEMPOTENT  Re-running produces byte-identical IDs. Documents that already have an id
              keep it untouched unless you pass --force.

  STATELESS   The projection seed and calibration sentences live in this source. The
              origin is regenerated from the embedding model, never learned from the
              corpus or stored as an irreplaceable numeric artifact.
...
# difference between 128 bits and 192 semantic bits, for free.
#
...
ID_VERSION = "v2"
ID_FORMAT = "simhash-sha256-rademacher-calibrated-192"
PROJECTION_SEED = f"semantic-ids/{ID_VERSION}/{ID_FORMAT}"

SEMANTIC_BITS = 192
TOTAL_BITS = SEMANTIC_BITS
ID_CHARS = TOTAL_BITS // 6  # 32

# A broad, reproducible zero point for the embedding space. These are part of the encoder,
# not a sample of any indexed corpus. Their embeddings are regenerated from the configured
# model, so there is no irreplaceable frozen numeric artifact.
CALIBRATION_TEXTS = (
    "Software tools, source code, and computer programs.",
    "Technical APIs, protocols, and reference documentation.",
    "Programming languages, compilers, and software architecture.",
    "Artificial intelligence, machine learning, and data models.",
    "Computer hardware, processors, storage, and operating systems.",
    "Networks, internet infrastructure, and distributed systems.",
    "Websites, browsers, publishing, and online services.",
    "Databases, information retrieval, and data analysis.",
    "Digital security, privacy, identity, and cryptography.",
    "Testing, debugging, maintenance, and technical support.",
    "Mobile devices, applications, cameras, and sensors.",
    "Cloud computing, deployment, automation, and system operations.",
    "Mathematics, logic, probability, and statistics.",
    "Physics, energy, matter, space, and time.",
    "Chemistry, materials, molecules, and laboratory science.",
    "Biology, genetics, evolution, and living systems.",
    "Medicine, health conditions, treatment, and patient care.",
    "Public health, nutrition, disease prevention, and safety.",
    "Psychology, emotion, cognition, behavior, and mental health.",
    "Climate, ecology, conservation, and the environment.",
    "Astronomy, planets, stars, and the universe.",
    "Engineering, machines, construction, and manufacturing.",
    "Agriculture, farming, food systems, and natural resources.",
    "History, archaeology, historical people, and past events.",
    "Geography, places, maps, countries, and cultures.",
    "Government, politics, public policy, and civic life.",
    "Law, courts, rights, regulation, and legal practice.",
    "Economics, markets, trade, labor, and public finance.",
    "Business, entrepreneurship, management, and organizations.",
    "Personal finance, banking, investing, insurance, and taxes.",
    "Education, schools, teaching, learning, and study.",
    "Philosophy, ethics, knowledge, meaning, and reasoning.",
    "Religion, spirituality, belief, ritual, and theology.",
    "Sociology, communities, inequality, institutions, and social life.",
    "Family life, kinship, caregiving, and household relationships.",
    "Friendship, romance, conflict, trust, and interpersonal relationships.",
    "Parenting, childhood, child development, and family support.",
    "Conversation, communication, correspondence, and social interaction.",
    "Work, careers, leadership, collaboration, and professional life.",
    "Community services, volunteering, local groups, and mutual aid.",
    "Travel, tourism, lodging, destinations, and trip planning.",
    "Homes, repairs, furniture, appliances, and domestic life.",
    "Gardening, plants, landscaping, and outdoor spaces.",
    "Cooking, recipes, ingredients, restaurants, and food culture.",
    "Fashion, clothing, beauty, grooming, and personal style.",
    "Literature, novels, poetry, storytelling, and literary criticism.",
    "Writing, editing, journalism, publishing, and rhetoric.",
    "Languages, linguistics, translation, grammar, and speech.",
    "Painting, sculpture, illustration, museums, and visual art.",
    "Design, typography, architecture, crafts, and aesthetics.",
    "Music, instruments, composition, performance, and audio.",
    "Film, television, animation, and visual storytelling.",
    "Photography, cameras, images, and visual archives.",
    "Theater, acting, dance, and live performance.",
    "Games, play, puzzles, interactive media, and recreation.",
    "Sports, teams, competition, athletes, and sporting events.",
    "Exercise, fitness, movement, training, and physical wellbeing.",
    "Hiking, camping, wilderness, parks, and outdoor recreation.",
    "Wellness, sleep, habits, meditation, and daily routines.",
    "Animals, pets, wildlife, and veterinary care.",
    "Transportation, vehicles, roads, aviation, and public transit.",
    "Emergencies, disasters, first aid, preparedness, and rescue.",
    "Disability, accessibility, assistive tools, and inclusive design.",
    "News, current events, media, and public information.",
    "Personal memories, journals, photographs, and life stories.",
)
...
def decode64(text: str) -> int:
    if len(text) != ID_CHARS or any(ch not in ALPHABET for ch in text):
        raise ValueError(f"expected {ID_CHARS} base64url characters, got {text!r}")
    bits = 0
    for ch in text:
...
def calibration_origin(vectors: list[list[float]]) -> list[float]:
    if len(vectors) != len(CALIBRATION_TEXTS):
        raise ValueError("calibration vector count does not match CALIBRATION_TEXTS")
    return [sum(values) / len(vectors) for values in zip(*vectors)]


@functools.cache
def projection_masks(dimensions: int) -> tuple[int, ...]:
    """Return one reproducible 256-bit sign mask per embedding dimension."""
    return tuple(
        int.from_bytes(hashlib.sha256(f"{PROJECTION_SEED}:{i}".encode()).digest(), "big")
        for i in range(dimensions)
    )


def binarize(vector: list[float], origin: list[float]) -> int:
    """
    Deterministic SimHash after subtracting a model-derived, corpus-independent origin.

    Raw origin-based SimHash is badly skewed because embedding spaces are not centered on
    numeric zero. The fixed calibration sentences supply a reconstructible zero point
    without turning one historical corpus snapshot into permanent encoder state.
    """
    if len(vector) != len(origin):
        raise ValueError("embedding and calibration origin dimensions differ")
    totals = [0.0] * SEMANTIC_BITS
    for value, center, mask in zip(vector, origin, projection_masks(len(vector))):
        delta = value - center
        for bit in range(SEMANTIC_BITS):
            totals[bit] += delta if mask & (1 << (255 - bit)) else -delta
    out = 0
    for total in totals:
        out = (out << 1) | (1 if total > 0 else 0)
    return out


def mint(vector: list[float], origin: list[float]) -> str:
    return f"{ID_VERSION}:{encode64(binarize(vector, origin))}"
...
def hamming(a: str, b: str) -> int:
    """
    Semantic distance. Versions must match: comparing codes from different encoders would
    produce a plausible-looking lie. Note there is no dash-stripping here. `-` is a
    legitimate base64url character. The
    ...
    bv, bbody = split_id(b)
    if av != bv:
        raise ValueError(f"cannot compare semantic IDs from {av} and {bv}")
    xor = decode64(abody) ^ decode64(bbody)
    if av == "v1":
        xor >>= 20  # v1's date/hash tail was not semantic
    return xor.bit_count()


def hamming_bits(a_bits: int, b: str) -> int:
    """Distance from an already-binarized query to a current semantic ID."""
    version, body = split_id(b)
    if version != ID_VERSION:
        raise ValueError(f"query uses {ID_VERSION}, index contains {version}")
    return (a_bits ^ decode64(body)).bit_count()
...
def yaml_list(items: list[str]) -> list[str]:
    ...
    This is not a style preference. An inline `key: ["a", "b"]` of two versioned IDs is
    over 90 characters, which is past a markdown formatter's 80-column print width — so prettier
...
# Directory names that say "documents live here" rather than what the documents are ABOUT.
# Never used as a title prefix; see read_doc.
GENERIC_DIRS = {"references", "reference", "docs", "doc"}
...
def read_doc(path: Path, src: dict, root: Path) -> dict | None:
    ...
    if parts:
        ...
        #
        # Skip generic container names on the way up. A shelf that lives INSIDE its skill
        # (skills/apple-silicon/references/foo.md) must still read "apple-silicon: ...",
        # never "references: ...". Two things break otherwise, and only one is obvious:
        # the prefix stops discriminating (every skill's shelf gets the same label), and
        # every already-stamped file's title silently changes, so the vector recomputed on
        # the next run no longer matches the semantic_id the file still carries. Moving a
        # shelf must not perturb its IDs.
        rel_parent = path.parent.relative_to(root)
        parts = [p for p in rel_parent.parts if p not in GENERIC_DIRS]
        if parts:
...
def find_context(name: str | None) -> Path:
    ...
    Resolution order, most specific first:
      ...
      (nothing)                   ./.agents/semantic-ids.toml walking up from cwd,
                                  then legacy ./.claude/semantic-ids.toml,
...
def load_context(name: str | None) -> dict:
    ...
    cfg.setdefault("related_max_distance", 60)
...
def cmd_mint(cfg: dict, args) -> None:
    ...
    if index_path.exists():
        prior = {
            str(Path(e["path"]).resolve()): e
            for e in json.loads(index_path.read_text())["docs"]
        }

    def key(d: dict) -> str:
        return str(d["path"].resolve())

    legacy = []
    for d in docs:
        existing = d["fields"].get("semantic_id") or prior.get(key(d), {}).get("id", "")
        if existing and split_id(existing)[0] != ID_VERSION:
            legacy.append(d)
    if legacy and not args.force:
        sys.exit(
            f"{len(legacy)} documents still carry v1 semantic IDs. The stateless encoder "
            "is v2; run `mint --force` for the deliberate one-time re-mint."
        )
    ...
    print(f"\nembedding {len(docs)} docs + {len(CALIBRATION_TEXTS)} fixed calibration "
          f"sentences with {cfg['embed_model']}...")
    embedded = embed(cfg, [*CALIBRATION_TEXTS, *[d["text"] for d in docs]])
    origin = calibration_origin(embedded[:len(CALIBRATION_TEXTS)])
    vectors = embedded[len(CALIBRATION_TEXTS):]
...
def cmd_query(cfg: dict, args) -> None:
    ...
    STAGE 1 — sweep with the bits. 192 bits is a COARSE instrument: a superb candidate
    ...
    precise, and it only ever touches the survivors. On the 2026-09 cached-corpus benchmark,
    the default top-80 shortlist retained the exhaustive cosine winner 97.4% of the time.
    """
    docs, metadata = load_index(cfg)
    query = " ".join(args.text)

    if metadata.get("id_version", "v1") != ID_VERSION:
        sys.exit(
            f"index uses {metadata.get('id_version', 'v1')}; query uses {ID_VERSION}. "
            "Run `mint --force` for the deliberate one-time re-mint."
        )
    ...
    if args.under:
        topic_version = cfg.get("topics", {}).get("version", "v1")
        want = args.under if ":" in args.under else f"{topic_version}:{args.under}"
    ...
    origin = calibration_origin(embed(cfg, list(CALIBRATION_TEXTS)))
    qv = embed(cfg, [query], query=True)[0]
    qbits = binarize(qv, origin)
...
def cmd_near(cfg: dict, args) -> None:
    ...
    hit = next((d for d in docs if str(Path(d["path"]).resolve()) == target), None)
    if not hit:
...
def cmd_stats(cfg: dict, args) -> None:
    ...
    information for this corpus.

    MEAN PAIR DISTANCE — should sit near half of the semantic bits. Two unrelated
    ...
    version = metadata.get("id_version", "v1")
    semantic_bits = metadata.get("semantic_bits", 172 if version == "v1" else SEMANTIC_BITS)
    ones = [0] * semantic_bits
    for i in ids:
        id_version, body = split_id(i)
        bits = decode64(body) >> (20 if id_version == "v1" else 0)
    ...
    pairs.sort()
    print(f"context {cfg['_name']!r}: {n} docs, {semantic_bits} semantic bits ({version})\n")
    print(f"  dead bits          {dead} / {semantic_bits}")
    ...
    if dead:
        print("\n  ! dead bits present — this corpus does not exercise the full encoder.")

    # Internal consistency cannot reveal a stale index. Compare canonical paths so
    # harness symlinks do not manufacture false additions and removals.
    live = {str(d["path"].resolve()) for d in collect(cfg)}
    indexed = {str(Path(d["path"]).resolve()) for d in docs}
    added, removed = live - indexed, indexed - live
    print(f"  corpus drift        {len(added)} unindexed, {len(removed)} removed")
    ...
    # a dangling ID is invisible by construction, since a versioned base64 string that
...
def main() -> None:
    ...
    q.add_argument("--shortlist", type=int, default=80, help="stage-1 candidates to rescore")

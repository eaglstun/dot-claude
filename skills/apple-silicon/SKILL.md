---
name: apple-silicon
description: Apple Silicon GPU and Metal compute reference for PyTorch MPS, CTranslate2, and standalone Metal projects. Use for MSL kernels, dispatch sizing, MPS GEMM/graphs, synchronization, memory, low-precision numerics, and Objective-C++ interop.
metadata:
  version: 1.0.0
  public: 'true'
  semantic_id: 9HHwmh88uNwk4rH9Wk5DNmZhlTIbsAAK
  related_ids: '["3HD6Cu8-sN6kzLF5Xk5DFiBl146dsAAI","3P98iM8kgN72r6S4Xs_jE2J1XwQb4AAL"]'
  topic_id: v2:NOHI
...
- **[references/pytorch-mps-backend-overview.md](references/pytorch-mps-backend-overview.md)**
  Apple's PyTorch-on-Mac landing page: the MPSGraph/tuned-kernel/custom-Metal architecture, consumer
  installation and availability checks, contribution routing, profiling/support links, and rules for
  handling its versioned requirements as a dated snapshot; read first for PyTorch MPS setup, backend
  selection, or support questions. PyTorch source-build work must still follow the target repo's build
  instructions.
...
- **[references/gpu-capture-and-shader-validation.md](references/gpu-capture-and-shader-validation.md)**
  ...
  Post-fault forensics, the other half of the file above: enhanced command buffer errors (`errorOptions = .encoderExecutionStatus` → which encoder `.faulted` vs was merely `.affected`), the `MTLCommandBufferError.Code` table, and GPU-side `os_log` shader logging (`MTLLogState`, Metal 3.2 / iOS 18); read when the GPU already faulted, hung, or returned garbage and you can't attach Xcode.
- **[references/ray-tracing-acceleration-structures.md](references/ray-tracing-acceleration-structures.md)**
  Metal ray tracing end to end: two-level acceleration structures (build/refit/compact and what each costs), instance masks and intersection-function tables, the MSL `intersector<>` vs `intersection_query<>` choice, `[[intersection(...)]]` functions, and the Apple6-works / **Apple9-is-hardware** split with no API to tell them apart; read before any ray-tracing work or when deciding whether a machine actually accelerates it.
- **[references/metal-cpp-bindings.md](references/metal-cpp-bindings.md)**
  Driving Metal from plain C++ instead of `.mm`: the `*_PRIVATE_IMPLEMENTATION` single-TU rule and how to read its three distinct link errors, selector→method naming rules, `NS::TransferPtr` vs `NS::RetainPtr`, and the big trap — **metal-cpp carries no availability annotations at all**, so a too-new API is a runtime crash, not a compile error. Verified by compiling; read before starting a C++ Metal project or debugging a metal-cpp link failure.
- **[references/metal-debugger-and-dependencies.md](references/metal-debugger-and-dependencies.md)**
  Reading a `.gputrace`: the Dependencies viewer (solid=data flow vs dashed=synchronization, and the tracked/untracked provenance asymmetry that tells you which dashed edges are yours), the sibling viewers with a per-viewer COMPUTE-applicability verdict, what each needs in the capture, and the headless-capture scope trap; read when a trace is open and you need to know which half of Apple's render-flavored tooling actually applies.
...
- **[references/metal4-tensors-and-mpp.md](references/metal4-tensors-and-mpp.md)**
  The Metal 4 (macOS 26) ML surface and THE find that MPP `matmul2d` supports char×char→int at base Metal 4 — an int8 matmul path that ties/beats MPS fp16; now CONFIRMED callable from an ordinary runtime-compiled compute pipeline (no Metal-4 host API, no MTLTensor, no explicit languageVersion), with the on-disk header locations and the exact `tensor_inline` / `slice<>` / element-type spellings that otherwise cost an afternoon — plus the NEGATIVE result that MPP only TIES MPS on float GEMM, so reach for it where MPS has a hole, not where MPS is mature; read alongside mpsndarray.md before any int8-GEMM rework.
...
- See the shelf README (`references/README.md`) for the add-a-topic recipes

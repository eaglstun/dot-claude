...
Example repo, `kernels/int_mm_mps.py`, 2026-08-10, M4 Max /
macOS 26.5.2 / torch 2.13. The tile design here ported cleanly to a second repo and a
second host language, which is decent evidence it is the right shape for the hardware
rather than a CT2 artifact.

- **The gap being filled is absence, not slowness.** PyTorch has no `aten::_int_mm` on the
  MPS dispatch key at all, so int8 checkpoints either raise `NotImplementedError` or fall
  back to the CPU per-linear (measured: **580 s per diffusion step**). Registering a Metal
  kernel from Python is enough to fix it — `torch.mps.compile_shader` for the MSL plus
  `torch.library.impl("aten::_int_mm", "MPS")` for the binding. No C++ extension.
- **The design transferred verbatim** and needed no retuning: 16×16 threadgroup, 64×64
  C-tile, BK=32, 4×4 register micro-tile, depth-major staging tiles, `char4`→`int4` inner
  loop, zero-pad on load / guard on store. Sustained **2.5–2.6 T-op/s**, essentially
  identical to CT2's ~2.4 T-MAC/s — the ALU-bound ceiling reproduces exactly.
- **Simplifications available when the contract is just `_int_mm`:** no alpha, no beta, no
  transpose flags, no u8-shift compensation. `_int_mm` is NN-only with an implicit
  alpha of 1, so the four-layout tile-load machinery and the integral-alpha routing guard
  are both unnecessary.
- **Confirms the "structurally 3–5× slower than fp16" claim** from outside CT2: 6.83 ms vs
  fp16's 1.52 ms at 2048³, a 4.5× gap. As there, **the fix is not a tiling tweak** — MPP
  `matmul2d` closed it to a tie at 1.39 ms (see `metal4-tensors-and-mpp.md`, which now
  carries the PyTorch-side MPP recipe and the corrected `tensor_inline` / `slice<>`
  spellings). The hand-tiled kernel is the fallback for pre-macOS-26.
- **PyTorch-specific gotchas:** only the functional `_int_mm` gets registered by
  `torch.library.impl`; the `.out` overload still raises. And a registered Python impl
  silently shadows `PYTORCH_ENABLE_MPS_FALLBACK`, so a wrong kernel produces wrong numbers
  rather than a slow-but-correct CPU result — parity suite before anything else.

...
`cuda-references` agent (`skills/cuda/references/`); engine structure is `ct2-internals`
...
there this shelf is `~/.claude/skills/apple-silicon/references/`.
...
- Keep SKILL.md lean: one-line pointers only. Detail lives in `~/.claude/skills/apple-silicon/references/`.
...
- To add a topic from Apple's **DocC docs** (Metal framework API — `MTLDevice`,
  ...
  `~/.claude/skills/apple-silicon/references/<topic>.md` with the source URL + a CT2-relevance section, and add a
  pointer above.
- To add a topic from the **MSL standard library** (kernel-side functions / language —
  ...
  `~/.claude/skills/apple-silicon/references/sources/Metal-Shading-Language-Specification.pdf` (WebFetch rejects it — >10MB; needs
  `poppler`). Recipe:
  ```bash
  pdftotext -layout ~/.claude/skills/apple-silicon/references/sources/Metal-Shading-Language-Specification.pdf /tmp/msl.txt
...
Carve these into `~/.claude/skills/apple-silicon/references/*.md` when a task needs them (CT2-relevance, in priority order).
Don't pre-build speculatively — pull on demand, same discipline as the rest of the backend.

- ~~**§6.8 SIMD-Group Matrix Functions**~~ — DONE: see `~/.claude/skills/apple-silicon/references/simdgroup-matrix-functions.md`
  (and the proven no-int8 fact that decided the int8 GEMM design).
- ~~**§6.16 Atomic Functions**~~ — DONE: see `~/.claude/skills/apple-silicon/references/atomic-functions.md`.
- ~~**§6.6 Math Functions**~~ — DONE: see `~/.claude/skills/apple-silicon/references/math-functions-and-numeric-parity.md`
...
- ~~**§6.10.1 / §4.4.1** Threadgroup & SIMD-group **synchronization**~~ — DONE: see
  `~/.claude/skills/apple-silicon/references/threadgroup-and-simdgroup-synchronization.md`.
- ~~**§6.3 Common Functions**~~ — DONE: see `~/.claude/skills/apple-silicon/references/common-functions.md` (with §6.5
  relational/select in `~/.claude/skills/apple-silicon/references/relational-and-select-functions.md`).

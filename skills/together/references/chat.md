...
| `thinkingmachines/Inkling`                | $1.00  | $4.05   | 512K | Murati's Thinking Machines open model (975B MoE, 41B active). **Reasoning model** — emits a `.reasoning` field; give it generous `max_tokens` or `content` comes back empty (see gotcha below). Long ctx, "customizable not strongest" positioning |
...
## Reasoning models: the `.reasoning` gotcha

`thinkingmachines/Inkling` (and any reasoning model) streams its chain-of-thought into a
separate `.choices[0].message.reasoning` field _before_ it writes `.content`. That thinking
counts against `max_tokens`. So a stingy cap gets you `finish_reason: "length"`, a full
`.reasoning` blob, and an **empty `.content`** — the extraction looks broken but the model
never reached the answer. Budget for it:

- Give `max_tokens` real headroom (a short answer still burned ~260 reasoning tokens in testing).
- `usage.completion_tokens_details.reasoning_tokens` tells you how much went to thinking.
- To inspect the thinking: `.choices[0].message.reasoning`. Normal answer is still `.content`.

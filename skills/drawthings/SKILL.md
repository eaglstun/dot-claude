---
name: drawthings
description: Generate images locally with the Draw Things macOS app's built-in HTTP API (AUTOMATIC1111-compatible). Use when the user wants to generate, img2img, or inpaint an image through Draw Things, or mentions the Draw Things API.
metadata:
  version: 1.0.0
  public: 'true'
  semantic_id: LI07MXMzQp_N2StDZ36nq6atCO-z8AAE
  related_ids: '["F6-pI0kqUJVt-Vl_7PalyvasfO8okAAI","sy09PWv50JsJSaMfYXWX2kbvWOb7AAAL"]'
  topic_id: v2:DFKI
...
- **[references/models.md](references/models.md)** - installed model/LoRA/ControlNet filenames (there's no API to list them), plus the per-model step/guidance settings. _Read when picking `--model`, `--lora`, or `--control`, and before setting `--steps`/`--guidance`._
...
- Default URL is `http://127.0.0.1:7860`. For another LAN device, set the app's IP to
  `0.0.0.0` and pass `--url http://<mac-host>:7860`.

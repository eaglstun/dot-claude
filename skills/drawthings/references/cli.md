...
draw-things-cli models import <checkpoint|safetensors>      # import a local artifact
                                                            # ⚠️ BASE MODELS ONLY — see below
```

### `models import` cannot import a LoRA ❌ (verified 2026-07-26, CLI `--version` = `dev`)

`models import` routes **every** artifact to `ModelImporter`, which only handles full
checkpoints. Hand it a LoRA `.safetensors` and it detects the family correctly
("Detected: FLUX.1"), prints `Importing: 5%`, then dies in ~0.5s with
`Error: tensorWritesFailed`, leaving a **useless 4096-byte `.ckpt` stub** you must delete
by hand. Proof it's a routing bug, not a bad file: `LoRAImporter.swift` throws only
`modelVersionFailed` — it never throws `tensorWritesFailed`, so the LoRA importer was
never reached. There is **no LoRA-import subcommand** (`models` has only
`list` / `ensure` / `import`).

Things that do **not** fix it: a clean `--models-dir` (same failure, so it isn't the
running app holding a lock), `--offline`, or normalizing the tensor key prefixes.

**Import LoRAs through the app UI** — that path calls `LoRAImporter` and works. Once
imported, the resulting `.ckpt` is usable headlessly via `--lora` forever after.

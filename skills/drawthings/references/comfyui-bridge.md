...
| ComfyUI (**upstream** `comfyanonymous`, not the stale drawthingsai fork) | `$COMFYUI_DIR`                                                                      |
| ComfyUI venv (Python 3.12, torch 2.12 + MPS)                             | `$COMFYUI_DIR/venv`                                                                 |
| Bridge node                                                              | `$COMFYUI_DIR/custom_nodes/draw-things-comfyui` (web assets prebuilt in `web/dist`) |
...
# 2) ComfyUI
cd "$COMFYUI_DIR" && ./venv/bin/python main.py --port 8188
#   → http://127.0.0.1:8188
...
through the bridge** (ComfyUI → gRPC → Z-Image Turbo → a clean fox in the snow).
`status: success`, no node errors. Generated test outputs are intentionally not committed.
...
- **Inpaint — ✅ WORKS.** `DrawThingsSampler` with `image` (fox) + `mask` (right third,
  ...
  **preserved** and only the masked region changed. HTTP
...
control_input_type="Custom", image=face)` → `control_net`, on Kolors. Redhead-ref → the
  ...
  HTTP/CLI produced unrelated faces.

...
def r_scope(W,H,fps,gain):
    # --smooth adds phosphor persistence, on the MASK, before the palette runs.
    # showwaves draws a fresh NON-OVERLAPPING audio window every frame, so the raw
    # output strobes ("frantic"). lagfun keeps a decaying ghost of prior frames.
    # It has to happen here, on gray8: smoothing the COLORISED video instead maxes
    # each RGB channel independently and leaves grey/pink smears. And use lagfun,
    # not tmix — the mask doubles as the alpha, so AVERAGING it makes peaks
    # translucent, which drops them down the height ramp and washes the palette
    # green. lagfun takes the max, so the live trace stays crisp and full-bright.
    sm = f",lagfun=decay={args.smooth:.3f}" if args.smooth > 0 else ""
    return (f"showwaves=s={W}x{H}:mode=line:draw=full:scale=sqrt:"
            f"rate={fps}:colors=white,format=gray8{sm}")
...
def main():
    ...
    ap.add_argument("--smooth", type=float, default=0.0,
                    help="scope only: phosphor-trail persistence 0..1 to calm the "
                         "motion (0=off/default, 0.88 is a good start, 0.93 calmer, "
                         ">0.95 fills into a solid slab). Applied to the mask before "
                         "the palette, so colours are preserved")

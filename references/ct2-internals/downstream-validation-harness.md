...
| -------------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| whisperx       | `<whisperx-venv>`                | conv stem + encoder, batched VAD-segmented inference (prefill-heavy). CT2 `WhisperModel` built on `device="metal"` and injected via `model=` because whisperX's torch-side VAD rejects "metal" (`tests/downstream/drivers/whisperx_driver.py:3-9,25-32`), `batch_size=8` (`:34`). |
...
- Model paths are machine-specific: configure the sample clip and int8 model directories
  in `projects.json`.

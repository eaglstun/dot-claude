...
description: >-
  ...
  `metal-renderer`), but it MAY save source-cited notes to `~/.agents/references/metal/`.
...
1. `AGENTS.md` or `CLAUDE.md` at the repo root — platform constraints, the clicker, `lensShift`/IPD. The
...
3. `~/.agents/skills/apple-arkit/references/scene-depth.md` + `camera-live-view.md` — what ARKit actually gives
...
This repo keeps curated, offline reference libraries under `~/.agents/references/`, tailored to
...
`~/.agents/references/metal/` the first time you have something worth saving there.**
...
phone-GPU cost you reasoned through — capture it as `~/.agents/references/metal/<topic>.md` so the
...
- **Low-level Metal API ground truth — the `apple-silicon` skill.** For storage modes / unified
  ...
  `apple-silicon` skill (`~/.agents/skills/apple-silicon/`) has condensed, Apple-source-cited
...
`~/.agents/references/metal/` when the research is worth keeping; note it if you did.

**Crosslinks (shared-shelf convention):** the reference shelves are one machine-wide library in `~/.agents/references/` (a git repo). Link across shelves as `[[shelf:file]]` (e.g. `[[apple-silicon:simd-group-functions]]`) in a `### See also` footer, and give every link a reason ("CUDA twin of…", "contradicts…"). Before writing a new GPU topic, check `~/.agents/references/gpu-rosetta.md` — the CUDA↔Metal concept map and convention spec. Keep MSL `[[attribute]]` syntax inside code spans so it does not parse as a link; `bash ~/.agents/references/audit-crosslinks.sh` verifies all links.

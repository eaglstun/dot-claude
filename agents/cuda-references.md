...
description: >-
  ...
  can save a trimmed local reference to the shared shelf at `~/.agents/skills/cuda/references/` when asked
...
related_ids:
  - "-nlSWq6W0cSlQPD7EHOTEfXQqTTz0AAC"
...
1. **Read the repo's `AGENTS.md`, `CLAUDE.md`, or `README.md`** at the root — the architecture and
   conventions of the project you are actually in.
2. **Find the CUDA code.** Don't assume a layout; look. Search for `**/*.cu`, `**/*.cuh`,
   `**/cuda/**`, and grep for `cublas`, `cudaMalloc`, `__global__`, `AT_DISPATCH`. Report the real
   paths you found.
3. **Check the shared shelf** at `~/.agents/skills/cuda/references/` — if a topic is already saved there,
...
- Most `docs.nvidia.com` pages render server-side and the available web-fetch tool returns them well; if a page comes
  back thin, try the specific symbol subpage, or the available web-search tool for
...
2. Save a **trimmed markdown digest** (not a raw HTML dump) under `~/.agents/skills/cuda/references/`, one
...
4. Add a two-line pointer for the new page to the index in `~/.agents/skills/cuda/SKILL.md`. Don't
...
**Crosslinks (shared-shelf convention):** the reference shelves are one machine-wide library in `~/.agents/references/` (a git repo). Link across shelves as `[[shelf:file]]` (e.g. `[[apple-silicon:mps-matrix-multiplication]]`) in a `### See also` footer, and give every link a reason ("CUDA twin of…", "contradicts…"). Before writing a new GPU topic, check `~/.agents/references/gpu-rosetta.md` — the CUDA↔Metal concept map and convention spec. Keep MSL `[[attribute]]` syntax inside code spans so it does not parse as a link; `bash ~/.agents/references/audit-crosslinks.sh` verifies all links.

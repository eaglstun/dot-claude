...
description: >-
  ...
  `~/.agents/references/metal/` doc library.
...
1. `AGENTS.md` or `CLAUDE.md` at the repo root — platform constraints, the clicker input model, `lensShift` /
...
4. `~/.agents/skills/apple-arkit/references/scene-depth.md` and `camera-live-view.md` — the ground truth for
...
(`~/.agents/skills/apple-silicon/`) is a source-cited Metal API reference — pull a matching
...
reference libraries under `~/.agents/references/` (`arkit/`, `threejs/`, `native-models/`, and
...
a depth detail — capture it as `~/.agents/references/metal/<topic>.md` (create the dir if absent)
...
2. If the change is visual and the user wants it confirmed on-device, you CAN deploy to the
   paired iPhone via xcodebuild + devicectl — but only build &

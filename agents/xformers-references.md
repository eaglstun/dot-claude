...
description: >-
  ...
  constraints, and can save trimmed local references under `~/.agents/skills/xformers/references/` when
...
related_ids:
  - "SQ6X4znNOKQlMBEDLUyKMCGiRfhSAAAL"
...
1. Any existing `~/.agents/skills/xformers/references/` you've saved before — reuse it instead of re-fetching.
...
- Most doc pages render fine via the available web-fetch tool; when the docs are thin or lag the code, read the GitHub
  source file directly (raw URL) and cite that. the available web-search tool `site:github.com/facebookresearch/xformers <symbol>`.
...
2. Save a **trimmed markdown digest** (not a raw HTML dump) under `~/.agents/skills/xformers/references/`,
...
3. Maintain `~/.agents/skills/xformers/SKILL.md` as a short index of what you've saved (matching

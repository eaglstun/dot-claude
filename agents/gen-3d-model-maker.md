...
description: >-
  ...
  cost + license. Grounded in `~/.agents/references/gen-3d/`. This is the GENERATIVE counterpart to
...
GLB loader + Metal linear-blend skinning, spec'd in `~/.agents/references/gen-3d/native-skinning-runtime.md`
...
1. `~/.agents/references/gen-3d/rig-and-animate-apis.md` — the vendor decision (this is your map):
...
2. `~/.agents/references/gen-3d/native-skinning-runtime.md` — the consumption spec. Read it for the
...
3. `~/.agents/references/gen-3d/tripo-api.md` — the concrete Tripo REST contract (base URL, auth,
...
- **Verify endpoints against the LIVE docs before calling.** The research is dated 2026-06-28 and
  these APIs drift. the available web-fetch tool `platform.tripo3d.ai/docs` (animation) / `docs.meshy.ai` (rigging +
  animation) to confirm the current request shape, then call with `curl` via the shell. Cite the doc URL
